...
class StoodleController extends StudipController
{
    ...
    public function display_action($id, $comments = null)
    {
        ...
        if ($stoodle->start_date && $stoodle->state > time()) {
            PageLayout::postError(_('Die Umfrage wurde noch nicht gestartet. Sie können noch nicht teilnehmen.'));
            $this->redirect('stoodle');
            return;
        }
        if ($stoodle->end_date && $stoodle->end_date < time()) {
            PageLayout::postError(_('Die Umfrage ist bereits beendet. Sie können nicht mehr teilnehmen.'));
            $this->redirect('stoodle');
            return;
        }
    ...
    public function participate_action($id)
    {
        ...
        PageLayout::postSuccess(_('Ihre Teilnahme wurde gespeichert.'));
    ...
    public function result_action($id)
    {
        $this->stoodle = new Stoodle($id);
        if (!$this->stoodle) {
            PageLayout::postError(_('Ungültige Stoodle-ID.'));
            $this->redirect('stoodle');
            return;
        }

        if (!$this->stoodle->evaluated) {
            PageLayout::postError(_('Die Umfrage ist noch nicht ausgewertet.'));
            $this->redirect('stoodle');
            return;
        }

        if (!$this->stoodle->is_public && !$GLOBALS['perm']->have_studip_perm('tutor', $this->range_id)) {
            PageLayout::postError(_('Die Umfrage ist nicht öffentlich. Sie haben keinen Zugriff auf diese Umfrage.'));
            $this->redirect('stoodle');
            return;
        }
